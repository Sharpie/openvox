...
def projects
  ...
  # about.
  output = Dir.chdir('packaging') { run_command('bundle exec vanagon list -r') }
  output.split("\n")[1..]
...
def component_info
  ...
  projects.each do |project|
    ...
    platforms.each do |platform|
      puts "  #{platform}"
      output = Dir.chdir('packaging') { run_command("bundle exec vanagon inspect #{project} #{platform}") } rescue next
...
namespace :vox do
  ...
  task :regenerate_component_info do
    ...
    run_command('git clone https://github.com/openvoxproject/openvox openvox-tmp')
    begin
      all_data = {}
      Dir.chdir('openvox-tmp') do
